...
const createStoryGraph = (client: OpenAI) => {
  ...
  const builderWithNodes = graphBuilder.addNode({
    ...
    brainstormConcepts: brainstormConceptsNode,
    writerOutline: writerOutlineNode,
    featureMuse: featureMuseNode,
    developerReview: developerReviewNode,
    writerApproval: writerApprovalNode,
    featureRewrite: featureRewriteNode,
    planPassages: pseudocodePlannerNode,
    generatePassages: generatePassagesNode,
...
export default defineEventHandler(async (event) => {
  ...
  const graph = createStoryGraph(client);
